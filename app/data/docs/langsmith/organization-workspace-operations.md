...
url: "https://docs.langchain.com/langsmith/organization-workspace-operations"
section: "langsmith"
last_modified: "2026-01-08T16:42:32.305Z"
---
This page provides a comprehensive reference table of [workspace](/langsmith/administration-overview#workspaces) and [organization](/langsmith/administration-overview#organizations) operations and which roles can perform them. The list includes API operations in LangSmith along with:
...
• [Roles and permissions](#roles-and-permissions): Custom roles
...
• [Access policies](#access-policies): Attribute-based access control
...
• [Usage and analytics](#usage-and-analytics): Usage tracking and TTL settings

**Workspace management:**  
• [Workspace settings](#workspace-settings-and-management): Members, settings  
• [Tags](#tags): Metadata tagging system  
• [Bulk Exports](#bulk-exports): Data export operations
...
Create org-scoped API key (workspace-scoped)\*
...
Create org-scoped API key (org-wide)\*
...
`organization:pats:create` + `organization:manage`
...
View project metadata (top K values)
...
Get specific insights job (Beta)
...
Send traces from SDK (includes single run, batch, multipart, and OTEL)
...
Generate filter query from natural language
...
Delete runs by trace ID or metadata
...
Download dataset (OpenAI format)
...
Download dataset (OpenAI fine-tuning format)
...
Run playground experiment (batch)
...
`prompts:read` + `datasets:read` + `projects:create`

Run playground experiment (stream)
...
View grouped examples with runs
...
`datasets:create` + `datasets:update` + `projects:create` + `runs:create`

Get experiment view overrides
...
Create experiment view override
...
Update experiment view override
...
Delete experiment view override
...
Create feedback with token (no auth required)
...
View pending workspace members
...
Delete pending workspace member
