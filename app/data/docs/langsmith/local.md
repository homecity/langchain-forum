...
url: "https://docs.langchain.com/langsmith/local"
section: "langsmith"
last_modified: "2025-12-16T22:07:24.461Z"
---
Sometimes it is helpful to run an evaluation locally without uploading any results to LangSmith. For example, if you’re quickly iterating on a prompt and want to smoke test it on a few examples, or if you’re validating that your target and evaluator functions are defined correctly, you may not want to record these evaluations. You can do this by using the LangSmith Python SDK and passing `upload_results=False` to `evaluate()` / `aevaluate()`. This will run you application and evaluators exactly as it always does and return the same output, but nothing will be recorded to LangSmith. This includes not just the experiment results but also the application and evaluator traces.
...
Let’s take a look at an example: Requires `langsmith>=0.2.0`. Example also uses `pandas`.
...
feedback.is\_concise

0

What is the largest mammal?

What is the largest mammal? is a good question. I don’t know the answer.
...
What do mammals and birds have in common? is a good question. I don’t know the answer.
