...
url: "https://docs.langchain.com/langsmith/agent-server-api/threads/join-thread-stream"
section: "langsmith/agent-server-api/threads"
last_modified: "2026-01-13T21:36:22.594Z"
---
...
```
curl --request GET \
  --url https://api.example.com/threads/{thread_id}/stream
```
...
Stream modes to control which events are returned. 'lifecycle' returns only run start/end events, 'run\_modes' returns all run events (default behavior), 'state\_update' returns only state update events.
