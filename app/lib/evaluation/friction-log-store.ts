...
const STORAGE_KEY = 'langchain-friction-log-v4'
...
const initialEntries: FrictionLogEntry[] = [
  ...
  // ============================================================================
  // Real-World Development Friction Points (from developer experience)
  // ============================================================================
  {
    id: '13',
    title: 'LCEL Chain Debugging Complexity',
    description: 'When composing chains with LCEL pipe operator (|), error stack traces show multiple layers of internal abstractions making it difficult to identify which step in the chain failed. Debugging requires peeling back each abstraction layer.',
    category: 'api',
    severity: 'high',
    status: 'open',
    component: 'LCEL / RunnableSequence',
    stepsToReproduce: '1. Create chain: retriever | prompt | llm | parser\n2. Force retriever to return empty results\n3. Observe error in downstream step\n4. Check stack trace - shows RunnableSequence internals',
    expectedBehavior: 'Error should clearly indicate which step failed and why',
    actualBehavior: 'Stack trace shows nested RunnableSequence calls, obscuring actual failure point',
    impactLevel: 'significant',
    impactDescription: 'Debugging takes 30+ minutes for simple chain errors',
    suggestedFix: 'Assign explicit names to each Runnable using .with_config(run_name="step_name") and enable LangSmith tracing for visual debugging',
    relatedDocs: 'https://python.langchain.com/docs/how_to/debugging',
    createdAt: '2026-01-15T20:00:00Z',
    updatedAt: '2026-01-15T20:00:00Z',
  },
  {
    id: '14',
    title: 'ConversationRetrievalChain Hidden Query Rephrasing',
    description: 'ConversationRetrievalChain internally rephrases user queries before retrieval without clear documentation. Follow-up questions like "tell me more about that" get aggressively rewritten, often losing original context and retrieving unrelated documents.',
    category: 'documentation',
    severity: 'medium',
    status: 'open',
    component: 'ConversationRetrievalChain',
    stepsToReproduce: '1. Set up ConversationRetrievalChain with vector store\n2. Ask: "How do I authenticate with LangSmith?"\n3. Follow up: "Tell me more about that"\n4. Check retrieved documents - often unrelated to original topic',
    expectedBehavior: 'Follow-up questions should maintain conversation context in retrieval',
    actualBehavior: 'Questions are overly rephrased, retrieving documents on different topics',
    impactLevel: 'significant',
    impactDescription: 'RAG response quality drops 50% on follow-up questions',
    suggestedFix: 'Customize condense_question_prompt template, or build custom chain with LCEL that gives explicit control over query transformation',
    relatedDocs: 'https://python.langchain.com/docs/how_to/chatbots_retrieval',
    createdAt: '2026-01-15T20:15:00Z',
    updatedAt: '2026-01-15T20:15:00Z',
  },
  {
    id: '15',
    title: 'Memory Component Excessive Token Usage',
    description: 'Default ConversationBufferMemory stores and sends entire conversation history with every request, causing token usage to grow linearly. Teams report 30%+ cost increase after extended conversations.',
    category: 'performance',
    severity: 'high',
    status: 'open',
    component: 'ConversationBufferMemory',
    stepsToReproduce: '1. Create chatbot with ConversationBufferMemory\n2. Conduct 20+ message conversation\n3. Check LangSmith trace for token count\n4. Observe token usage grows with each message',
    expectedBehavior: 'Memory should use efficient windowing or summarization by default',
    actualBehavior: 'Full history sent every time, wasting tokens on old context',
    impactLevel: 'significant',
    impactDescription: 'Monthly API cost increased from $500 to $650 (30%)',
    suggestedFix: 'Use ConversationBufferWindowMemory(k=10) or ConversationSummaryMemory to limit context size',
    relatedDocs: 'https://python.langchain.com/docs/how_to/chatbots_memory',
    createdAt: '2026-01-15T20:30:00Z',
    updatedAt: '2026-01-15T20:30:00Z',
  },
  {
    id: '16',
    title: 'Multiple Implementation Patterns Cause Confusion',
    description: 'The same functionality can be implemented multiple ways in LangChain, with documentation showing different approaches. For retrievers alone: as_retriever(), VectorStoreRetriever, BaseRetriever subclass, RunnableLambda wrapping. No clear guidance on which is recommended.',
    category: 'documentation',
    severity: 'medium',
    status: 'open',
    component: 'Retriever / Documentation',
    stepsToReproduce: '1. Search "langchain custom retriever"\n2. Find official docs, tutorials, GitHub examples\n3. Each shows different implementation approach\n4. Unclear which is current best practice',
    expectedBehavior: 'Documentation should clearly state recommended pattern with migration guides',
    actualBehavior: 'Multiple approaches coexist with deprecated code mixed in examples',
    impactLevel: 'moderate',
    impactDescription: 'Spent 2 hours deciding implementation approach, refactored twice',
    suggestedFix: 'Follow newest docs pattern, check langchain version compatibility, prefer LCEL-based approaches for new code',
    relatedDocs: 'https://python.langchain.com/docs/how_to/custom_retriever',
    createdAt: '2026-01-15T20:45:00Z',
    updatedAt: '2026-01-15T20:45:00Z',
  },
  {
    id: '17',
    title: 'Streaming and Callback Behavior Inconsistency',
    description: 'Callback handlers behave differently depending on streaming mode. on_llm_new_token only fires when streaming=True. Non-streaming mode only triggers on_llm_end. This requires branching logic for consistent observability.',
    category: 'api',
    severity: 'medium',
    status: 'open',
    component: 'CallbackManager / Streaming',
    stepsToReproduce: '1. Create custom CallbackHandler with on_llm_new_token and on_llm_end\n2. Call LLM with streaming=False - on_llm_new_token never fires\n3. Call LLM with streaming=True - on_llm_new_token fires per token\n4. Must implement both code paths for logging',
    expectedBehavior: 'Callbacks should fire consistently regardless of streaming mode',
    actualBehavior: 'Different callbacks fire depending on streaming setting',
    impactLevel: 'moderate',
    impactDescription: 'Observability code complexity doubled with branching logic',
    suggestedFix: 'Use LangSmith tracing instead of custom callbacks, or implement separate handlers for streaming/non-streaming modes',
    relatedDocs: 'https://python.langchain.com/docs/how_to/streaming',
